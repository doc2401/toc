...
Get-ChildItem -Directory -Filter "spring-data-*" | ForEach-Object {
    # 用原目录名创建外层包装目录，例如 spring-data-commons -> 00.spring-data-commons
    $wrapper = Join-Path $_.Parent.FullName "00.$($_.Name)"

    # 确保外层目录存在
    New-Item -ItemType Directory -Path $wrapper -Force | Out-Null

    # 把原 spring-data-* 目录移动到外层目录里，形成多一层嵌套
    Move-Item -LiteralPath $_.FullName -Destination $wrapper
}
...
Get-ChildItem -Directory -Filter "00*" | ForEach-Object {
    ...
    pw2401 dir-copy  -Extension html -DeleteOriginal -ContainsText "Generated by javadoc"
    Set-Location ..
} 
...
Get-ChildItem -Directory -Filter "00.*.copy" | ForEach-Object {
    # 00.spring-data-mongodb.copy -> 01.spring-data-mongodb.javadoc
    $newName = $_.Name -replace '^00\.', '01.' -replace '\.copy$', '.javadoc'
...
## 重命名提取文件
Get-ChildItem -Directory -Filter "00.*.copy" | ForEach-Object { 
    $newName = $_.Name -replace '^00\.', '01.' -replace '\.copy$', '.html'
    Rename-Item -LiteralPath $_.FullName -NewName $newName
}
